...
public record MaterialIconType(String name) {
    ...
    public ResourceLocation getBlockTexturePath(MaterialIconSet materialIconSet, String suffix,
                                                boolean doReadCache) {
        if (doReadCache) {
    ...
    public ResourceLocation getItemTexturePath(MaterialIconSet materialIconSet, String suffix,
                                               boolean doReadCache) {
        if (doReadCache) {
            if (suffix == null || suffix.isBlank()) {
        ...
        while (!iconSet.isRootIconset) {
            ResourceLocation location = GTCEu.id(String.format("textures/item/material_sets/%s/%s%s.png", iconSet.name, this.name, suffix));
            if (ResourceHelper.isResourceExist(location)) {
                break;
            }
            iconSet = iconSet.parentIconset;
